---
title: "Kalfírvach v1.4 — Guía para Agentes"
version: v1.4
date: 2026-06-11
lang: es
status: stable
...
# KALFÍRVACH v1.4 — Guía para Agentes Autónomos
...
| Archivo léxico | `kalfirvach_lexicon_v1.4.json` |
| Glosario | `glosario.md` (1.118 entradas, 3 índices) |
| Gramática | `gramatica_v1.4.md` (4.656 líneas) |
| Fonología | `fonologia_v1.4.md` (1.298 líneas) |
| Diccionario HTML | `diccionario_kalfírvach_v1.4.html` (con escritura griega politónica) |
...
```
Kalfirvach_v1.3/
├── kalfirvach_lexicon_v1.4.json        ← FUENTE DE VERDAD del léxico
├── glosario.md                          ← Reflejo EXACTO del JSON (regenerable vía json2md_lexicon.py)
├── gramatica_v1.4.md                    ← Gramática completa con ejemplos
├── fonologia_v1.4.md                    ← Fonología, fonotáctica, prosodia
├── guía_kalfírvach_para_agentes_v1.4.md ← ESTE DOCUMENTO
├── diccionario_kalfírvach_v1.4.html     ← Diccionario HTML con script griego politónico
...
1. **`kalfirvach_lexicon_v1.4.json` es la fuente de verdad.** Todo cambio léxico va aquí primero. `glosario.md` se regenera desde el JSON, no se edita a mano.
2. Los ejemplos en `gramatica_v1.4.md` deben reflejar el léxico actual. Si se cambia una palabra en el JSON, revisar la gramática por ejemplos que la usen.
...
**NUNCA** escribir IPA /v/ para palabras con `v` en LA. Siempre /w/. Este error ocurrió en ~29 entradas y se corrigió en la limpieza v1.4.
...
El archivo `diccionario_kalfírvach_v1.4.html` carga `lexicon_data.js` (generado desde el JSON) y renderiza cada entrada con:
...
```python
import json
with open('kalfirvach_lexicon_v1.4.json') as f:
    data = json.load(f)
...
El HTML referencia `LEXICON_DATA["kalfirvach_lexicon_v1.4"].categorias`. Si se cambia la key del JSON, hay que actualizar el HTML.
...
| lexicon_data.js v0.2 obsoleto | JS para HTML | Regenerado desde v1.4 |
| diccionario.html apuntaba a v0.2 | Referencia en HTML | Actualizado a v1.4 |
...
## 11. Cambios Clave de v1.4 Respecto a Versiones Anteriores
...
| Diccionario HTML | Actualizado a v1.4 con script griego politónico |
...
*Documento generado como guía de referencia rápida para agentes autónomos que trabajen con Kalfírvach v1.4. Versión: 2026-06-11.*
